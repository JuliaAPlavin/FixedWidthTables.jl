...
@testitem "read StructArray" begin
    import FixedWidthTables as FWT
    using StructArrays

    a = FWT.read(joinpath(@__DIR__, "rfc_2020b_cat.txt"), StructArray,
        (
            cat = (1:1, Char),
            name = (4:11, String),
            cnt = (80:85, Int),
    ...
    @test isequal(a, FWT.read(joinpath(@__DIR__, "rfc_2020b_cat.txt"),
        (
            cat = (1:1, Char),
            name = (4:11, String),
            cnt = (80:85, Int),
...
@testitem "read DictArray" begin
    import FixedWidthTables as FWT
    using DictArrays
    using DictArrays.Dictionaries

    a = FWT.read(joinpath(@__DIR__, "rfc_2020b_cat.txt"), DictArray,
        (
            cat = (1:1, Char),
            name = (4:11, String),
            cnt = (80:85, Int),
    ...
    @test length(a) == 73
    @test length(unique(collect(a))) == 73
    @test isequal(a[1], dictionary([:cat => 'N', :name => "2357-141", :cnt => 17, :s_flux => missing]))
    @test isequal(a.s_flux, [missing, missing, missing, missing, missing, 0.569, 0.115, missing, 0.035, missing, 0.842, 0.327, missing, 0.149, missing, 0.319, missing, missing, missing, 0.204, missing, missing, missing, 0.018, missing, missing, missing, 0.009, missing, missing, missing, missing, missing, missing, missing, 0.056, 0.006, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, 0.227, 0.183, 0.164, missing, 0.203, missing, missing, 0.07, missing, missing, missing, 0.297, missing, missing, missing, missing, missing, missing, 0.633, missing, missing, missing, missing])
    @test eltype(a) <: AbstractDictionary{Symbol}
end

@testitem "multiple line variants" begin
    import FixedWidthTables as FWT
    using StructArrays
    using DictArrays

    e = FWT.read(joinpath(@__DIR__, "cats_table.txt"),
        [
    ...
    @test isequal(
        FWT.read(joinpath(@__DIR__, "cats_table.txt"), DictArray,
        [
            (cat=(1:5, String),name=(8:27, String),RA=(29:40, String),eRA=(43:47, Float64),Dec=(50:61, String),eDec=(63:67, String),freq=(69:76, Float64),flux=(78:86, Float64),flux_err=(88:95, Float64),_=(97:97, String)),
            (cat=(1:5, String),name=(8:28, String),RA=(30:41, String),eRA=(44:48, Float64),Dec=(51:62, String),eDec=(64:68, String),freq=(70:77, Float64),flux=(79:87, Float64),flux_err=(89:96, Float64),_=(98:98, String)),
        ];
        skiprows_startwith="#", restrict_remaining_chars=' ', missings=["n"]) |> StructArray,
        e
    )
end
...
@testitem "validation" begin
    import FixedWidthTables as FWT
    @test_throws "shorter than expected" FWT.read(joinpath(@__DIR__, "rfc_2020b_cat.txt"),
        (
            cat = (1:1, Char),
            name = (4:11, String),
            cnt = (80:85, Int),
...
@testitem "autodetect" begin
    import FixedWidthTables as FWT
    using StructArrays
    using DictArrays

    b = FWT.read(joinpath(@__DIR__, "0005m23.umrao"), skiprows=1:1, headerrow=2)
    ...
    c = FWT.read(joinpath(@__DIR__, "S_0048-097.txt"), skiprows=1:1, headerrow=2)
    @test keys(c[1]) == (:YYYYMMDD, Symbol("Freq."), Symbol("U.T."), :S, :SigS, :N, :_field_7, :_field_8, :_field_9)
    @test c.S == ["1.62", "0.10"]
    @test isconcretetype(eltype(c))

    d = FWT.read(joinpath(@__DIR__, "rfc_2021a_z.txt"),
    ...
    @test isequal(
        FWT.read(joinpath(@__DIR__, "0005m23.umrao"), DictArray, skiprows=1:1, headerrow=2) |> StructArray,
        b
    )
    @test isequal(
        FWT.read(joinpath(@__DIR__, "S_0048-097.txt"), DictArray, skiprows=1:1, headerrow=2) |> StructArray,
        c
    )
    @test isequal(
        FWT.read(joinpath(@__DIR__, "rfc_2021a_z.txt"), DictArray,
            headerrow="J2000      B1950    | UNK| fullname                      |ratext      |dectext     |class |  redshift|flag|redshift_ref       ",
            delim=[' ', '|'], missings=[""]) |> StructArray,
        d
    )
end
...
@testitem "_" begin
    ...
    CHL.@check()
    Aqua.test_all(FixedWidthTables, ambiguities=false)
